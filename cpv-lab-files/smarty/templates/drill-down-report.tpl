		    ...
		    <div class="card-body">
		        ...
		        <div class="mt-4">
                    ...
                    <div class="ddr-selectors d-flex flex-wrap align-items-center justify-content-start mb-3" style="float:left">
                        ...
                        <button onclick="refreshButton(); return false;" class="btn btn-primary ml-3" data-toggle="tooltip" data-placement="top" title="Refresh"><i class="fa fa-refresh"></i> Refresh</button>
...
<script type="text/javascript">
	{literal}
		...
		{
			window.location.href = getFullUrl(false);
		}

		function getFullUrl(withLevels)
		{
            var levels = [];
            for (i=1; i<=4; i++)
            {
                if ($("#ddlDrilldownLevel" + i).val() > 0)
                {
                    levels.push($("#ddlDrilldownLevel" + i).val());
                }
            }
            
			redirectUrl = 'drill-down-report.php?camp=' + $("#ddlCampaigns").val() +
					"&int=" + $("#ddlInterval").val() +
					($("#ddlInterval").val() == 4 ? "&intfrom=" + encodeURIComponent($("#txtIntervalFrom").val()) +
							"&intto=" + encodeURIComponent($("#txtIntervalTo").val()) : '') +
                    (withLevels ? "&level=" + levels.toString() : '');
					//"&level1=" + $("#ddlDrilldownLevel1").val() + "&level2=" + $("#ddlDrilldownLevel2").val() +
                    //"&level3=" + $("#ddlDrilldownLevel3").val() + "&level4=" + $("#ddlDrilldownLevel4").val();
			return redirectUrl;
		}

        function refreshButton()
		{
			window.location.href = getFullUrl(true);
		}
